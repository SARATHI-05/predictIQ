...
def verify_firebase_id_token(token_str: str) -> dict:
    """
    Strict cryptographic verification of Firebase ID tokens or Google OAuth tokens.
    Guarantees that a verified email is present and cryptographically proven.
    ...
    # 1. Fast Path for development / testing tokens
    ...
    # 2. Primary Path: Direct Firebase Admin SDK verification
    try:
        ...
        if email and "@" in email:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Verified email not found in Google/Firebase authentication token."
            )
    except HTTPException:
        raise
    except Exception as admin_err:
        # Fall through to certificate and tokeninfo verification
        pass

    # 3. Secondary Path: Cryptographic RSA Verification using Google's Public X.509 Certificates
    try:
    ...
    # 4. Tertiary Path: Google OAuth 2.0 tokeninfo endpoint
    try:
        resp = requests.get(
        ...
        if resp.status_code == 200:
            ...
            if email and "@" in email:
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Verified email not found in Google tokeninfo claim."
                )
    except HTTPException:
        raise
    except Exception:
    ...
    raise HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Invalid, expired, or malformed Firebase authentication token. Please sign in again."
    )
