...
def test_email_endpoint(to: str = "predictiqfoodmanagement@gmail.com"):
    ...
    safe_config = {
        ...
        "password_len": len(config["password"]),
    }
    ...
    for p in ports_to_try:
        try:
            if p == 465:
                ...
                with smtplib.SMTP_SSL(config["host"], 465, context=ssl_ctx, timeout=10) as server:
                    server.login(config["user"], config["password"])
                    msg = MIMEText("Test from Render live backend", "plain")
                    msg["Subject"] = "Test SMTP from Render"
                ...
                import smtplib
                from email.mime.text import MIMEText
                with smtplib.SMTP(config["host"], 587, timeout=10) as server:
                    server.starttls()
                    server.login(config["user"], config["password"])
                    msg = MIMEText("Test from Render live backend", "plain")
                    msg["Subject"] = "Test SMTP from Render"
            ...
            errors.append(f"Port {p}: {type(e).__name__}: {str(e)}")
